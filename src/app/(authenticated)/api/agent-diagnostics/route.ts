import "server-only";
// IMPORTANT: force System-Assigned Managed Identity
...
/** Temporarily mask OpenAI key env vars so Projects factory uses MI only; restore afterwards */
  ...
  try {
    ...
    if (!endpoint || endpoint.trim().length === 0) {
      return new Response(JSON.stringify({
        error: "Missing AZURE_AIPROJECT_ENDPOINT (or AZURE_AI_PROJECT_ENDPOINT_STRING).",
    ...
    // Force System-Assigned Managed Identity (SAMI) — avoids UAMI/CLI confusion
    const miCredential = new ManagedIdentityCredential();

    let projectClient: AIProjectClient;
    try {
      projectClient = new AIProjectClient(endpoint, miCredential);
    } catch (e: any) {
      restore();
      return new Response(JSON.stringify({
        error: `Failed to create AIProjectClient (MI): ${e?.message ?? String(e)}`
      }), { status: 500, headers: { "Content-Type": "application/json" } });
    }
    ...
    // Candidate agent payloads
    const formsToTry: any[] = [];
    if (configuredId.length > 0) {
      formsToTry.push(configuredId);
    ...
    // Get OpenAI client via Project, using the same MI
    ...
    try {
      if (typeof (projectClient as any).getOpenAIClient === "function") {
        ...
        throw new Error("Neither getOpenAIClient() nor getAzureOpenAIClient() exists on AIProjectClient.");
      }
    } catch (e: any) {
      restore();
      return new Response(JSON.stringify({
        error: `Failed to get OpenAI client (MI): ${e?.message ?? String(e)}`
      }), { status: 500, headers: { "Content-Type": "application/json" } });
    }
    ...
    try {
      // Pattern B: direct responses.create (no conversation)
