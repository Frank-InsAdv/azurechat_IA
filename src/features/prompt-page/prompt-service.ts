...
export const CreatePrompt = async (
  ...
  try {
    ...
    if (!user) {
      return {
        status: "UNAUTHORIZED",
        errors: [{ message: "You must be logged in to create a prompt." }],
      };
    }
    ...
    return {
      status: "ERROR",
      errors: [{ message: `Error creating prompt: ${error}` }],
    };
  }
};
...
> => {
  try {
    ...
    return {
      status: "ERROR",
      errors: [{ message: `Error retrieving prompt: ${error}` }],
    };
  }
};

/**
 * Ensures the current user can operate on a prompt
 * - Admins can operate on any
 * - Users can only operate on their own prompts
 */
export const EnsurePromptOperation = async (
  ...
  if (
    promptResponse.status === "OK" &&
    (currentUser.isAdmin ||
      promptResponse.response.userId === (await userHashedId()))
  ) {
  ...
  return {
    status: "UNAUTHORIZED",
    errors: [{ message: `Not authorized to access prompt: ${promptId}` }],
  };
};
...
export const DeletePrompt = async (
  ...
  try {
    ...
    if (promptResponse.status === "OK") {
      ...
      return { status: "OK", response: deletedPrompt };
    }
    ...
    return {
      status: "ERROR",
      errors: [{ message: `Error deleting prompt: ${error}` }],
    };
  }
};
...
export const FindPromptByID = async (
  ...
  try {
    ...
    return {
      status: "ERROR",
      errors: [{ message: `Error finding prompt: ${error}` }],
    };
  }
};
...
export const UpsertPrompt = async (
  ...
  try {
    ...
    if (promptResponse.status === "OK") {
      ...
      const modelToUpdate: PromptModel = {
        ...
        isPublished: user.isAdmin
          ...
          : prompt.isPublished, // users cannot toggle publish
        createdAt: new Date(),
      };
    ...
    return {
      status: "ERROR",
      errors: [{ message: `Error updating prompt: ${error}` }],
    };
  }
};
